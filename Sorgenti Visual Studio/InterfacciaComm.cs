    ...
    public class InterfacciaComm
    {
        ...
        private bool RIFcomFTDI(string Descrittore, uint baudrate)
        {
            if (!myFtdiDevice.IsOpen)
            {
                ...
                {
                    monitor.ScriviSuMonitor(String.Format("Interfaccia con descrittore \"{0}\" connessa a: {1}", Descrittore, baudrate));
        ...
        private FTDI.FT_STATUS OpenFTDI(string Descrizione, uint baudrate)
        {
            ...
            uint ndev = 0;
            myFtdiDevice.GetNumberOfDevices(ref ndev);

            if (ndev > 0)
            {
                FTDI.FT_DEVICE_INFO_NODE[] devicelist = new FTDI.FT_DEVICE_INFO_NODE[ndev];
                myFtdiDevice.GetDeviceList(devicelist);
                int nDescUguali = 0;
                for (int i = 0; i < devicelist.Length; i++)
                {
                    if (devicelist[i].Description == Descrizione)
                    {
                        nDescUguali++;
                        monitor.ScriviSuMonitor("Trovata interfaccia FTDI con descrittore \"{0}\" sulla porta USB n:{1} (Location)", Descrizione, devicelist[i].LocId);
                    }
                }
                if (nDescUguali > 0)
                {
                    if (nDescUguali > 1)
                        ftStatus = myFtdiDevice.OpenByLocation(Location);
                    else
                    ...
                    {
                        if (nDescUguali>1)
                            monitor.ScriviSuMonitor("Collegata interfaccia FTDI che si trova sulla porta USB n:{0} (Location)", Location);
