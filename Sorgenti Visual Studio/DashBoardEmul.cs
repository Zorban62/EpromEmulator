...
namespace Andrea_NameSpace
{
    ...
    public partial class DashBoardEmulEP : Form
    {
        ...
        public DashBoardEmulEP()
        {
            ...
            InterfEmulEP.Location = Convert.ToUInt32(preference.FTDIlocation);
        ...
        private void toolStripTxtBoxDescrittore_TextChanged(object sender, EventArgs e)
        {
            InterfEmulEP.DisConnetti();
        }
        ...
        private void txbAsmFile_Click(object sender, EventArgs e)
        {
            ...
            if (openFileDialog2.ShowDialog() == DialogResult.OK)
            {
                ...
                if(chbAutoAssembler.Checked) btnCompilaZasm.PerformClick();
            }
        }
        ...
        {
            monitor.ClearMonitor();
            if (CPUinRun) ResetCPU(true); //Ferma emulazione
        ...
        private bool LoadFileHex(string NomeFile)
        {
            ...
            else
            {
                ...
                btnLoadAndRun.Enabled = true;
                monitor.ScriviSuMonitor("Letto file Hex:  {0}", txbHexFile.Text);
        ...
        private void HexChange(object source, FileSystemEventArgs e)
        {
            ...
            if (fireCount == 1)
            {
                ...
                if (File.Exists(File_Path_Hex))
                {
                    ...
                    {
                        DialogResult result = MessageBox.Show(this, "File " + txbHexFile.Text + " modificato vuoi rileggerlo?", "File Change", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                        if (result == System.Windows.Forms.DialogResult.Yes)
                        {
                            if (!LoadFileHex(File_Path_Hex))
                            {
        ...
        private bool LoadFileBin(string NomeFile)
        {
            ...
            try
            {
                ...
                monitor.ScriviSuMonitor("Letto file Bin: {0}", txbBinFile.Text);
        ...
        private void BinChange(object source, FileSystemEventArgs e)
        {
            ...
            if (fireCount == 1)
            {
                ...
                if (File.Exists(File_Path_Bin))
                {
                    ...
                    {
                        DialogResult result = MessageBox.Show(this, "File " + txbBinFile.Text + " modificato vuoi rileggerlo?", "File Change", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
        ...
        {
            if ( ResetCPU(CPUinRun) != ErrorNodo.NO_ERROR)
            {
                string msg;
                if (CPUinRun) msg = "Non è stato possibile resettare la CPU!";
                else msg = "Non è stato possibile avviare la CPU!";
        ...
        {
            if (rst) monitor.ScriviSuMonitor("\n---> Invio comando di Reset CPU");
            else monitor.ScriviSuMonitor("\n---> Invio comando di Run CPU");
            FrameInterfaccia frm = new FrameInterfaccia
            {
            ...
            FrameInterfaccia ris = InterfEmulEP.InviaFrame(frm, true);
            if (ris == null || ris.Comando != ComFrame.COM_SET_COMMAND || ris.Msg[0] != 0)
            {
                lblStatoCPU.Visible = false;
                return ErrorNodo.NODO_NON_RISPONDE;
            }
            CPUinRun = !rst; //Aggiorna stato
            lblStatoCPU.Visible = true;
            if (rst)
            {
                lblStatoCPU.BackColor = System.Drawing.Color.Red;
                btnResetRun.Text = "RUN CPU";
                lblStatoCPU.Text = "CPU in RESET";
                monitor.ScriviSuMonitor("CPU in Reset");
            }
            else
            {
                lblStatoCPU.BackColor = System.Drawing.Color.LightGreen;
                btnResetRun.Text = "RESET CPU";
                lblStatoCPU.Text = "CPU in RUN";
                monitor.ScriviSuMonitor("CPU in Run");
                SystemSounds.Beep.Play();
            }
            return ErrorNodo.NO_ERROR;
        }
        ...
        private void btnLoadEmul_Click(object sender, EventArgs e)
        {
            ...
            { 
                monitor.ScriviSuMonitor("Trasferimento dati su emulatore avvenuto con successo.");
                btnResetRun.Enabled = true;
            }
        }
        ...
        private void LoadPreference()
        {
            try
            {
                ...
                toolStripFTDIlocation.Text = preference.FTDIlocation;

                monitor.ScriviSuMonitor("Preferenze caricate da Preference.xml");
            }
            catch 
            {
        ...
        private void SavePreference()
        {
            ...
            preference.FTDIlocation = toolStripFTDIlocation.Text;
        ...
        private void istruzioniToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            try
            {
                using (Process AsmProc = new Process())
                {
                    AsmProc.StartInfo.FileName = @"Istruzioni Dashboard EmulEP.pdf";
                    //AsmProc.StartInfo.WorkingDirectory = Path.GetDirectoryName(AsmFilePath);
                    AsmProc.Start();
                }
            }
            ...
            {
                monitor.ScriviSuMonitor(exc.Message.ToString());
                MessageBox.Show(this, exc.Message.ToString(), "Errore di lettura file", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        private void schemaElettricoToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                using (Process AsmProc = new Process())
                {
                    AsmProc.StartInfo.FileName = @"Schema.pdf";
                    //AsmProc.StartInfo.WorkingDirectory = Path.GetDirectoryName(AsmFilePath);
                    AsmProc.Start();
                }
            }
        ...
        //-------------------------------------------------------------------------------------------------------------
        /// <summary> Modificata location FTDI <summary>
        //-------------------------------------------------------------------------------------------------------------
        private void toolStripFTDIlocation_TextChanged(object sender, EventArgs e)
        {
            uint FTDI_Location;
            if (UInt32.TryParse(toolStripFTDIlocation.Text, out FTDI_Location))
            {
                InterfEmulEP.Location = FTDI_Location;
                InterfEmulEP.DisConnetti();
            }
        }
